    ...
    handleAddButton: (addButton, collection) => {
        addButton.addEventListener('click', function () {
            const isArrayCollection =
                collection.classList.contains('field-array')
            // Use a counter to avoid having the same index more than once
            let numItems = parseInt(collection.dataset.numItems)

            // Remove the 'Empty Collection' badge, if present
            const emptyCollectionBadge =
                this.parentElement.querySelector('.collection-empty')
            ...
            const formTypeNamePlaceholder =
                collection.dataset.formTypeNamePlaceholder
            const labelRegexp = new RegExp(
                formTypeNamePlaceholder + 'label__',
                'g'
            )
            ...
            const collectionItemsWrapper = collection.querySelector(
                newItemInsertionSelector
            )

            collectionItemsWrapper.insertAdjacentHTML('beforeend', newItemHtml)

            // Execute JS scripts embedded in prototype
            const collectionItems = collectionItemsWrapper.querySelectorAll(
            ...
            if (!isArrayCollection) {
                EaCollectionProperty.updateCollectionItemCssClasses(collection)
                const lastElementCollapseButton =
                    lastElement.querySelector('.accordion-button')
                lastElementCollapseButton.classList.remove('collapsed')
                const lastElementBody = lastElement.querySelector(
                    '.accordion-collapse'
                )
    ...
    updateCollectionItemCssClasses: (collection) => {
        ...
        collectionItems.forEach((item) =>
            item.classList.remove(
                'field-collection-item-first',
                'field-collection-item-last'
            )
        )
