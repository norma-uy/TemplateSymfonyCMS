...
class TextEditorField {
    ...
    #processRequiredAttribute() {
        ...
        document
            .querySelectorAll('textarea.ea-text-editor-content')
            .forEach((trixContentElement) => {
                const trixEditorConfig = JSON.parse(
                    trixContentElement.getAttribute('data-trix-editor-config')
                )
                if (null !== trixEditorConfig) {
                    Trix.config = this.#mergeObjects(
                        trixEditorConfig,
                        Trix.config
                    )
                }

                const isTrixFieldRequired =
                    'required' === trixContentElement.getAttribute('required')
                        ? 'true'
                        : 'false'
                trixContentElement.setAttribute(
                    'data-ea-trix-is-required',
                    isTrixFieldRequired
                )
                ...
                if (trixContentElement.dataset.numberOfRows !== '') {
                    const editor = document.querySelector(
                        `trix-editor[input=${trixContentElement.id}].trix-content`
                    )
                    ...
                    if (editor !== null) {
                        ...
                        editor.style.setProperty(
                            'min-height',
                            `${21 * trixContentElement.dataset.numberOfRows}px`
                        )
                    }
                }
            })
    ...
    #handleFormSubmission() {
        document.addEventListener('ea.form.submit', (formEvent) => {
            ...
            entityForm
                .querySelectorAll('textarea.ea-text-editor-content')
                .forEach(function (trixContentElement) {
                    const isTrixFieldRequired =
                        'true' ===
                        trixContentElement.getAttribute(
                            'data-ea-trix-is-required'
                        )
                    const trixEditorElement = entityForm.querySelector(
                        `trix-editor[input=${trixContentElement.id}]`
                    )
                    // an empty Trix editor field is not really empty; it contains a "\n" character (%0A = HTML encoded)
                    const isTrixEditorEmpty =
                        '%0A' ===
                        escape(
                            trixEditorElement.editor.getDocument().toString()
                        )

                    if (isTrixFieldRequired && isTrixEditorEmpty) {
                        const formGroup =
                            trixContentElement.closest('div.form-group')
                        ...
                        const errorMessage =
                            TextEditorField.#getLocalizedErrorMessage()
                ...
                form.querySelectorAll('input,select,textarea').forEach(
                    (input) => {
                        if (
                            ...
                            input.getAttribute('data-ea-trix-is-required') ===
                                'true' &&
                            input.value === ''
                        ) {
